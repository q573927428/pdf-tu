...
def generate_cover(settings: Settings, copy: str, name: str, category: str, grade_subject: str,
    ...
    composition_options = (
        "一张大幅练习纸作为视觉主体，搭配环绕式小图标与手绘批注",
        "打开的书本或作业本居中展开，知识图形从页面向外自然延伸",
        "三至五张大小不同的知识卡片错落拼贴，形成清晰的疏密节奏",
        "文件夹半开并露出多张资料页，以分层纸张和索引标签组织画面",
        "用圆形、方格或路径式信息图串联知识点，搭配少量学习文具",
        "以本学科代表性教具或知识图形为中心，便签和纸胶带只作陪衬",
        "纵向学习清单配合局部放大的教材细节，利用留白形成轻盈版面",
        "左右不对称构图：一侧为主资料页，另一侧用小图形和文具平衡画面",
    )
    composition_key = "|".join((name, category, grade_subject, semester))
    composition_index = int(hashlib.sha256(composition_key.encode("utf-8")).hexdigest()[:8], 16) % len(composition_options)
    composition = composition_options[composition_index]
...
- 底部头像区（最高优先级）：参考图中的圆形头像/徽章是不可修改的固定素材，必须原样保留。头像放在年级科目标签下方、底部提示文字上方的独立留白区域内；头像中心与画面垂直中轴线重合，并在该留白区域内水平、垂直双向居中。不得偏左、偏右、贴边或与其他元素重叠。
【顶部标题区——位置永远固定，样式与文字随文案可变】
- 位置固定：标题始终位于画面顶部约15%高度的居中区域，标题下方保留一条鹅黄色手绘波浪下划线
- 文字极简：只提取一个2—6个中文字的核心短语，优先使用名词或短动宾词组；必须单行显示，不加副标题、标点、年级、册别或解释性文字，不重复资料名称
- 样式克制：只使用清晰的大号深棕色手写字，可配一层很淡的荧光笔底色或简洁圆角底框；禁止叠加印章、贴纸字、多个标签和折行
【可变段——中部内容区，自由发挥，不固定模板】
本张优先构图方向：“{composition}”。将它作为灵感而非固定模板，可根据资料内容继续调整。
中部占画面约55%—65%，必须成为主要视觉区域。结合学科内容灵活选用知识图形、几何图形、统计图、数字符号、字词卡、时间轴、思维导图、练习纸、书本、文件夹、铅笔、尺规或其他相关元素；不同封面应改变主体类型、卡片数量、大小、位置、层叠关系、倾斜角度和留白比例。图形内容应与本期资料相关，允许一幅主图、多图标环绕、路径式排布、网格卡片或不对称拼贴等变化。避免机械复刻参考图，尤其不要默认使用“双便签卡片+中央三角尺”的固定组合。资料名称与亮点要有清晰主次、方便阅读；年级科目和册别统一放在中下部的米黄色横条标签卡中，位置可有小幅变化但需保持稳定识别。
...
【规则】所有待渲染中文用“”标出并写清位置；全图不超过4处主要文字块，顶部标题2—6字且单行，其他每处≤10字；文字清晰可读、笔画完整；严禁水印、平台logo。背景质感、主色调、字体气质和信息层级保持统一，版面结构、主体图形、卡片形态、装饰元素与留白方式要自然变化；画面要像同一套系列资料，但不要每张都长得一样。"""
...
function filteredRows(){const q=document.getElementById('search').value.toLowerCase(),category=document.getElementById('category-filter').value,subject=document.getElementById('subject-filter').value,showPublished=document.getElementById('show-published').checked;return [...document.querySelectorAll('#rows tr')].filter(r=>(!category||r.cells[4]?.textContent.trim()===category)&&(!subject||r.cells[3]?.textContent.trim()===subject)&&(showPublished||r.querySelector('.status-btn')?.dataset.status!=='已发布')&&r.innerText.toLowerCase().includes(q))}
function initCategoryFilter(){const select=document.getElementById('category-filter'),categories=[...new Set([...document.querySelectorAll('#rows tr')].map(r=>r.cells[4]?.textContent.trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'zh-CN'));categories.forEach(category=>select.add(new Option(category,category)))}
function initSubjectFilter(){const select=document.getElementById('subject-filter'),subjects=[...new Set([...document.querySelectorAll('#rows tr')].map(r=>r.cells[3]?.textContent.trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'zh-CN'));subjects.forEach(subject=>select.add(new Option(subject,subject)))}
